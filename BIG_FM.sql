...
SELECT COUNT(a.awardid) AS No_of_Awards, s.showname 
FROM awards AS a
LEFT JOIN shows AS s on a.showid = s.showid
GROUP BY a.showid, s.showname;

--4)Find shows that have partnerships with "Spotify"?

SELECT p.partnername, s.showname
FROM shows as s
JOIN showpartnerships as sp ON s.showid = sp.showid
JOIN partnerships as p ON sp.partnershipid = p.partnershipid
WHERE p.partnername = 'Spotify';

--5)Retrieve hosts who joined before 2010?

SELECT * FROM hosts
WHERE DATE(joindate) < '2010-01-01';

--6)List the shows and their launch dates in descending order of launch date?

SELECT showname, date(launchdate)
FROM shows
ORDER BY DATE(launchdate) desc;
...
SELECT stationname 
FROM stations
WHERE location = 'South' AND date(launchdate) < '2010-01-01';
